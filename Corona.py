...
def run_game():
    ...
    output.close()
    # name = "N{0}; P{1}; K{2}.txt".format(initial_n, p,k)
    graphCreator(file_str)
...
def graphCreator(filename):
    ...
    plt.title("Number of Infected Cells Vs Generation")
    plt.plot(x, y)
    plt.show()
    ...
    for row in range(rows):
        for col in range(cols):
            if grid[row][col] == 0:
                ...
                # output_str += "@ "
                output_str += "\033[92m{}\033[00m".format("@ ")
            else:
                output_str += "\033[91m{}\033[00m".format("X ")
        output_str += "\n\r"
